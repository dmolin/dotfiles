...
THIRD_DISPLAY=`xrandr --listactivemonitors | grep "2:" | awk '{print $NF}'`


if [ $MONITORS -eq 1 ]; then
  # only 1 display
  TOP_DISPLAY=$MAIN_DISPLAY
  THIRD_DISPLAY=$MAIN_DISPLAY
fi

export MAIN_DISPLAY=$MAIN_DISPLAY 
export TOP_DISPLAY=$TOP_DISPLAY
export THIRD_DISPLAY=$THIRD_DISPLAY

echo $MAIN_DISPLAY $TOP_DISPLAY $THIRD_DISPLAY
