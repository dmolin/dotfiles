...
# -*- coding: UTF-8 -*-
##############################################################################
# Forked from https://github.com/loopzen/pomobar
...
# DEFAULT CONFIGS
ICON_TOTAL = " "
CONFIG_FOLDER = HOME_FOLDER + "./.config/pomobar"
DATABASE_PATH = CONFIG_FOLDER + "/pomobar.db"

# Create config director if not there already
if not os.path.exists(CONFIG_FOLDER):
  os.makedirs(CONFIG_FOLDER)

# CUSTOM CONFIGS
config = configparser.ConfigParser()
config.read(CONFIG_FOLDER + '/pomobar.conf')
...
class Pomodoro:
    ...
    def start(self):
        os.system("notify-send --urgency=normal Pomodoro started")
        while self.duration >= 0:
            writeOutput(self.ICON + str(self.duration))
            recharge_polybar()
            time.sleep(MINUTE)
            self.duration -= 1
            #os.system("notify-send --urgency=normal Pomodoro ticking")
...
def dispatcher(args):
    #os.system("notify-send --urgency=low Starting dispatcher...")
...
def main():
    #os.system("notify-send Starting pomobar...")
    """Start programm"""
